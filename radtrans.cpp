...
CRadTrans::CRadTrans()
{
	...
	absorp = 0.85;   // leaf absorptivity for PAR (higher value reduce how much light penetrates deeper
	clump = 1.0;     // clumped means randomly distributed
	rho_soil = 0.10; // soil reflectivity for PAR band, reflected light condibution to shaded leaves
 ...
 // the transmittance values obtained from Day and Bailey (1999), chap 3, ecosystems of the world 20: greenhouse ecosystem, page 76
	...
	Kb(GetZenith()); // Calculate  KbVal extiction coefficient
...
void CRadTrans::Kd(double LA) // apparent extinction coefficient for diffuse radiation
{
...
double CRadTrans::Qsl() // mean flux density on sunlit leaves
 {
  return GetKb()*IrradianceDirect + Qsh(); //sunlit receives both direct radiation as well as diffuse radiation
 }

double CRadTrans::Qsl(double L) // flux density on sunlit leaves at delpth L
 {
  // IrradianceDirect: direct pfd
  // Qsh: flux density on shaded leaves
  return GetKb()*IrradianceDirect + Qsh(L);
 }

double CRadTrans::Qsh() // mean flux density on shaded leaves over LAI
 {
  //Qdm= absorbed diffused flux
  //Qsc= weighted average scattered radiation within canopy
  //Qsoilm= Soil reflectance
