...
#define new DEBUG_NEW
	...
	void CGasExchange::SetVal(double PhotoFluxDensity, double Tair, double CO2, double RH, double wind, double LAI, double Press, bool ConstantTemperature,
...

	{
		...
		this->SIF_GasEx = SIF_GasEx;
		this->sifvalue  = sifvalue;
		this->qL = qL;
		this->kdf = kdf;
		this->phiPS2max = phiPS2max;
		this->fesc = fesc;
		...
		//** \code
	{
		...
		if (SIF_GasEx == 1)
		{
			if (sifvalue != 0) J = qL * (phiPS2max * (1 + kdf) / ((1 - phiPS2max) * fesc)) * sifvalue;
			else J = 0.0; 
			
			Aj = J * (Cc - gamma) / (4 * (Cc + 2 * gamma));
			AssimilationNet = Aj - DarkRespiration;
			Ac = 0.0;
			AjValue = 00;
			AcValue = Ac;
			ApValue = 0.0;
		...
		*/
	{
		...
		if (SIF_GasEx == 1)
		{
			...
			if (sifvalue != 0) J = qL * phiPS2max * (1 + kdf) / (1 - phiPS2max) * sifvalue/fesc;
			else J = 0.0;
			AssimilationGross = (1 - x) / 3 * J;
			AssimilationNet = AssimilationGross - DarkRespiration;
		...
		//*! \fn CalcStomatalConductance()
	{
		...
		double Ds, //! \b Ds, VPD at leaf surface 
			...
			//Gamma = 10.0; removing becuase we want to use the gamma from c3photosynthesis
			//** \endcode
		double Gamma = GammaValue;//Gamma, CO2 compensation point in the absence of mitochondirial respiration
